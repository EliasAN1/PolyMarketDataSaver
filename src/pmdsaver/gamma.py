...
from pmdsaver.clock import Window
...
def extract_resolved_outcome(event: dict[str, Any]) -> str | None:
    """Authoritative UP/DOWN winner from Gamma once Polymarket has resolved the market.

    After close, ``markets[0].outcomePrices`` is ``["1","0"]`` or ``["0","1"]``
    aligned with ``outcomes`` (``["Up","Down"]``). Live books still show mid-range
    prices like 0.505 / 0.495 — those must not be treated as a winner.
    """
    markets = event.get("markets") or []
